...
function setup() {
  const canvas = createCanvas(document.querySelector("#gameArea").clientWidth, document.querySelector("#gameArea").clientHeight);
  canvas.parent("gameArea");

  input = createInput();
  input.id("answerInput");
  input.parent(document.body);  // 캔버스 바깥 body에 넣고
  input.attribute("placeholder", "정답을 입력하고 Enter");

  positionInputBox(); // 🔹 위치 설정!
...
function positionInputBox() {
  const canvasBox = document.querySelector("#gameArea").getBoundingClientRect();
  input.position(canvasBox.left, canvasBox.bottom + window.scrollY);
  input.size(canvasBox.width);
}

function windowResized() {
  resizeCanvas(document.querySelector("#gameArea").clientWidth, document.querySelector("#gameArea").clientHeight);
  positionInputBox();
}
